...
public class DishController {
    ...
    public ResponseEntity<DisplayDishDto> findByIdWithDetails(@PathVariable Long id) {
        return dishApplicationService.findByIdWithDetails(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<DisplayDishDto> update(@PathVariable Long id, @RequestBody CreateDishDto createMenuItemDto) {
        return dishApplicationService.update(id, createMenuItemDto).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }

    @DeleteMapping("/{id}/delete")
    public ResponseEntity<DisplayDishDto> deleteById(@PathVariable Long id) {
        return dishApplicationService.deleteById(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }

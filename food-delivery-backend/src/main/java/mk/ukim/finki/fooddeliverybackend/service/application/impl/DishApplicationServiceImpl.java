...
public class DishApplicationServiceImpl implements DishApplicationService {
    ...
    @Override
    public Optional<DisplayDishDto> findByIdWithDetails(Long id) {
        return dishService.findById(id).map(DisplayDishDto::from);
    }

    @Override
    public DisplayDishDto save(CreateDishDto createDishDto) {
        Restaurant restaurant = restaurantService.findById(createDishDto.restaurantId()).orElseThrow(
                () -> new RestaurantNotFoundException(createDishDto.restaurantId()));
